...
optionsManifest = u.readJson(__dirname + '/manifest.json');
...
module.exports = {
    ...
    allOptions: function() {
        ...
        if ( !out ) {
            out = {};
            u.merge(out, optionsManifest.cli);
            u.merge(out, optionsManifest.main);
            allOptions = out;
        }
        return out;
    },
    ...
    optionsMap: function() {
        var out = optionsMap;
        if ( !out ) {
            optionsMap = u.transform(optionsManifest.main, function(acc, its, ix){
                var
                    aliasArr = its.alias,
                    alias,
                    def = its.default,
