...
module.exports = {
    ...
    filterUnknown: function (options) {
        ...
        _.forOwn(options, function(it, ix){
            if ( !_.has(optionsDic.cliOnly, ix) && !_.has(optionsDic.cliAndRc, ix) ) {
                out.push(ix);
            }
        });
    ...
    lookup: function (filename) {
        ...
        return isFile? fs.readFileSync(fullpath): null;
    },

    externalRcOptions: function (path) {
        return this.parseRc(this.lookup(path));
    }
};
